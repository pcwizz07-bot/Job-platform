...
export default function Login({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.requires_2fa) { setTempToken(res.temp_token); setStep2FA(true) }
      else onLogin(res.user, res.token)
  ...
  const handle2FA = async (e) => {
    ...
    try { const res = await api.verify2fa(tempToken, code); onLogin(res.user, res.token) }
    catch (err) { setError(err.message) }
    setBusy(false)
  }
      ...
      <div className="login-card">
        ...
        <form onSubmit={handle2FA}>
          <div className="form-group"><label>Code</label>
            <input value={code} onChange={e => setCode(e.target.value)} placeholder="000000" required maxLength={6} /></div>
          <button type="submit" style={{ width: '100%' }} disabled={busy}>{busy ? 'Verifying...' : 'Verify'}</button>
        </form>
      </div>
    </div>
  )
